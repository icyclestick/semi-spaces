...
public class DungeonBuilder : MonoBehaviour
{
    ...
    public void Generate()
    {
        ...
        while (frontier.Count > 0 && iterations < targetIterations)
        {
            ...
            // --- Build the corridor (1 fewer segment so it stops before the gate) ---
            int buildLen = Mathf.Max(1, finalLen - 1);
            generator.BuildCorridor(gate.position, gate.direction, buildLen, parent);
            corridorsBuilt++;

            Debug.Log($"[DungeonBuilder] Corridor from ({gate.position.x:F0},{gate.position.z:F0}) " +
                      $"dir ({gate.direction.x:F0},{gate.direction.z:F0}), length {buildLen} (gate fills last tile).");
